...
class Toy(nn.Module):
    ...
    def forward(self, parameters):
        ...
        #alpha = self.noise_std/0.02
        alpha = 0.5
        beta = torch.Tensor([0.2, 1])+(self.design-0.4)*alpha
        true_obs = parameters**3*beta+parameters*torch.exp(-torch.abs(0.2-beta))+(self.design)**2
