...
def compute_posterior_entropy(method, noise_std): 
    if method == 'gradbed':
        ...
        with open("results/STATS5/even_gradbed_noise_std_{0}.pkl".format(noise_std), "rb") as file:
            results = pickle.load(file)
    elif method == 'ace':
        torch.manual_seed(2377)
        try:
            with open("results/STATS5/even_ace_noise_std_{0}.pkl".format(noise_std), "rb") as file:
                results = pickle.load(file)
        except:
            results = None
    elif method == 'mcmc':
        torch.manual_seed(2377)
        with open("results/STATS5/even_mcmc_noise_std_{0}.pkl".format(noise_std), "rb") as file:
            results = pickle.load(file)
    elif method == 'apnmc':
        torch.manual_seed(2377)
        with open("results/STATS5/even_apnmc_noise_std_{0}.pkl".format(noise_std), "rb") as file:
            results= pickle.load(file)
    elif method == 'pce':
        torch.manual_seed(2377)
        with open("results/STATS5/even_pce_noise_std_{0}.pkl".format(noise_std), "rb") as file:
            results = pickle.load(file)
    ...
    if results is None:
        ...
        with open("results/STATS5/posterior_val_{0}_noise_std_{1}_even.pkl".format(method, noise_std), "wb") as file:
