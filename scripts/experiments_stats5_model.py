...
init_design = STATS5(torch.Tensor([1])).default_times
...
if __name__ == "__main__":
    ...
    def run_experiments(method, noise_std):
        if method == 'gradbed':
            ...
            with open("results/STATS5/gradbed_noise_std_{0}.pkl".format(noise_std), "wb") as file:
                pickle.dump(gradbed_results, file)
        elif method == 'ace':
            torch.manual_seed(2377)
            try:
                ace_results = ace_design([init_design, noise_std])
                with open("results/STATS5/ace_noise_std_{0}.pkl".format(noise_std), "wb") as file:
            ...
            with open("results/STATS5/mcmc_noise_std_{0}.pkl".format(noise_std), "wb") as file:
                pickle.dump(mcmc_results, file)
        elif method == 'apnmc':
            torch.manual_seed(2377)
            apnmc_results = apnmc_design([init_design, noise_std])
            with open("results/STATS5/apnmc_noise_std_{0}.pkl".format(noise_std), "wb") as file:
                pickle.dump(apnmc_results, file)
        elif method == 'pce':
            torch.manual_seed(2377)
            pce_results = vnmc_design([init_design, noise_std])
            with open("results/STATS5/pce_noise_std_{0}.pkl".format(noise_std), "wb") as file:
